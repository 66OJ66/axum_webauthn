...
pub fn prepare_postgres_connection_string() -> Result<String, ()>{
    let postgres_user: String = std::env::var("POSTGRES_USER").unwrap_or_else(|_| DEFAULT_POSTGRES_USER.to_string());

    let postgres_password: String = std::env::var("POSTGRES_PASSWORD").unwrap_or_else(|_| DEFAULT_POSTGRES_PASSWORD.to_string());

    let ip_address: String = std::env::var("POSTGRES_IP").unwrap_or_else(|_| DEFAULT_POSTGRES_IP.to_string());
    ...
    Ok(format!("postgres://{0}:{1}@{2}:{3}", postgres_user, postgres_password, ip_address, port))
}